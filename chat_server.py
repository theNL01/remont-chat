...
ТОЛЬКО когда есть имя, контакт (телефон или email) И способ связи — добавь в конце:
...
def chat():
    ...
    if "[ЗАЯВКА:" in reply:
        try:
            ...
            if is_valid_name(name) and is_valid_contact(contact) and connection:
                history = format_history(messages)
                send_telegram(
                    f"🔔 *Новая заявка с сайта!*\n\n"
                    f"👤 *Имя:* {name}\n"
                    f"📞 *Контакт:* {contact}\n"
                    f"💬 *Способ связи:* {connection}\n\n"
                    f"📋 *Переписка:*\n{history}\n\n"
