...
ВАЖНО: Ты консультант по ремонту, а не собеседник. Если клиент пишет не по теме (привет, как дела, расскажи анекдот и т.д.) — вежливо верни разговор к теме ремонта. Не поддерживай посторонние разговоры.
...
Если клиент интересуется конкретной услугой или хочет узнать стоимость — предложи оставить заявку и попроси имя и номер телефона.

ТОЛЬКО когда клиент назвал настоящее имя (минимум 4 буквы, например "Иван" или "Александр") И номер телефона — добавь в конце ответа тег:
[ЗАЯВКА: имя=ИМЯ, телефон=ТЕЛЕФОН]

Не ставь тег если нет обоих: имени и телефона. Не придумывай данные клиента."""
...
def chat():
    ...
    # Проверяем тег заявки — только если есть имя И телефон
    if "[ЗАЯВКА:" in reply:
        try:
