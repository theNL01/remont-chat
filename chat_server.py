...
2. Клиент назвал имя → обратись по имени БЕЗ приветствия (не "Здравствуйте", не "Привет"), ответь на вопрос и уточни детали: что именно нужно сделать, какая площадь, когда планируют начать. Приветствие только в самом первом сообщении когда спрашиваешь имя.
3. Клиент рассказал детали → спроси как удобнее связаться:
...
[ЗАЯВКА: имя=ИМЯ, контакт=КОНТАКТ, связь=СПОСОБ_СВЯЗИ, детали=ДЕТАЛИ_РЕМОНТА]

Способ связи необязателен — если не указан пиши "не указан".
Детали — кратко что клиент рассказал о ремонте (вид работ, площадь, сроки). Если не рассказал — оставь пустым.
    ...
    if "[ЗАЯВКА:" in reply:
        try:
            ...
            if is_valid_name(name) and is_valid_contact(contact):
                ...
                if details:
                    comment = f"{comment}. {details}".strip(". ")
                send_to_sheets(name, contact, comment, source="чат сайта", is_chat=True)
