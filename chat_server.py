...
4. В зависимости от выбора спроси только нужное:
   - Позвонить / Перезвонить → спроси номер телефона коротко, обратившись по РЕАЛЬНОМУ имени клиента из этого диалога (НЕ "Даша" и не любое другое случайное имя - только то имя, которое клиент сам назвал ранее), например: "[Имя клиента], продиктуйте ваш номер - перезвоним в удобное время."
...
Если клиент в процессе интервью спрашивает про цены на наши услуги - ответь конкретной цифрой из списка ниже, а не общими оценочными цифрами, затем верни разговор к текущему вопросу анкеты:
{{УСЛУГИ_И_ЦЕНЫ}}"""
...
def interview():
    ...
    system = INTERVIEW_PROMPT.replace("{{УСЛУГИ_И_ЦЕНЫ}}", build_services_block())
    if name:
...
Актуальные цены на наши услуги (используй их, если разговор зайдёт о стоимости работ компании):
{{УСЛУГИ_И_ЦЕНЫ}}"""
